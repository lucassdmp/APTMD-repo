...
function field_user()
{
    ...
    if (isset($_POST['submitsocioemail'])) {
        ...
        foreach($socios as $soc){
            ...
            $email = sanitize_email($soc);
            $formador2 = get_user_by('email', $email);
            $max = intval($wpdb->get_var("SELECT MAX(CAST(meta_value as UNSIGNED)) FROM $wpdb->usermeta WHERE meta_key = 'Socio'"))+1;
            update_user_meta($formador2->ID, 'Socio', $max);
        }
                
        //create an wordpress user with $socio email
        // $user_id = username_exists($socio);
        // if (!$user_id and email_exists($socio) == false) {
        //     $random_password = '1234socio';
        //     $user_id = wp_create_user($socio, $random_password, $socio);
        //     update_user_meta($user_id, 'Socio Type', $socio_type);
        // }
...
?>
        <form class="form-container" action="" method="post">
            ...
            <input type="email" name="socio" id="socio"><br>
            <label for="socios">INSIRA TODOS OS SOCIOS PARA DAR O TIPO CERTO!</label>
            <input type="text" name="socios" id="socios"><br>
            <label></label>
            <?php if ($error === 1) : ?>
                <div class="error">Utilizador Não Encontrado!</div>
            <?php endif; ?>
            ...
            <select name="socio-type" id="socio-type">
                ...
                <option value="terapeuta-2">Sócio Terapeuta 2 Semestres</option>
                <option value="amigo-1">Sócio Amigo 1 Semestre</option>
                <option value="amigo-2">Sócio Amigo 2 Semestres</option>
